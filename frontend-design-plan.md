...
3. **Bklit UI chart swap — PAUSED, not finished (2026-08-10).** Replace Recharts in the 3 Dashboard chart components (SpendingChart→`area-chart`, IncomeExpenseChart→`bar-chart`, CategoryChart→`ring-chart`/`pie-chart`) with Bklit UI's real registry components. Investigated their actual `registry.json` (github.com/bklit/bklit-ui) — real matches exist for all 3, but the dependency footprint is much heavier than expected: built on `@visx/*` + `d3-array`, plus Bklit's own chart-context/animation/tooltip/axis infrastructure shared across every chart type, and its own OKLCH color-token system that would need reskinning (same class of issue as Kokonut's dropdown-menu semantic tokens). User was torn on which charts to pick and asked to defer — **revisit this explicitly before considering the frontend design plan complete**, don't silently drop it.
...
- ✅ DONE (2026-08-10, commit `6b84a61`) — ChatGPT competitive-research feature gaps: new Reports page (`src/pages/Reports.tsx`) with invoice aging, client-level income tracking, and cash flow forecast. Consolidated reports view was the 4th gap and is effectively covered by this new page.
