...
import { Plus, Trash2, AlertCircle, Check, Pencil } from 'lucide-react'
...
export function PayoutBucketsEditor({ defaultTaxRate, defaultSavingsRate }: PayoutBucketsEditorProps) {
  ...
  // Nothing saved yet means nothing to accidentally overwrite, so the form
  // shows directly — the edit-lock only matters once real buckets exist.
  const showForm = editing || (!isLoading && buckets.length === 0)
  ...
  function handleCancel() {
    reset(bucketsToFormValues(buckets))
    setSaveError(null)
    setEditing(false)
  }
      ...
      {!showForm ? (
        <div className="flex flex-col gap-2">
          {buckets.map((bucket, i) => {
            ...
            return (
              <div key={bucket.id} className={cn('flex items-center gap-2.5 px-3 py-2.5 rounded-xl', style.iconBg)}>
                <Icon className={cn('w-4 h-4 shrink-0', style.iconColor)} />
                <p className="flex-1 min-w-0 text-sm font-medium text-gray-900 truncate">{bucket.name}</p>
                <span className="text-sm font-semibold text-gray-700 shrink-0">
                  {bucket.percentage != null ? `${bucket.percentage}%` : 'Remainder'}
                </span>
              </div>
            )
          })}
