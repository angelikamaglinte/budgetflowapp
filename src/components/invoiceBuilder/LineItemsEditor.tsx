...
export function LineItemsEditor({ control, register, errors }: LineItemsEditorProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {fields.map((field, index) => (
        <div key={field.id} className="grid grid-cols-12 gap-2 p-3 bg-gray-50 rounded-xl">
          ...
          <div className="col-span-6 sm:col-span-3">
            <input
              {...register(`line_items.${index}.date_range`)}
              placeholder="Date range"
              className="w-full px-2.5 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            />
          </div>
      ...
      <button
        type="button"
        onClick={() => append({ description: '', date_range: '', qty: 1, rate: 0 })}
