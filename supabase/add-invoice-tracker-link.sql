-- Links Invoice Builder to the Invoices tracker: every new invoice created
-- there (manually, or automatically via a recurring invoice template) now
-- also creates a matching "Pending" row in the invoices tracker, so it
-- shows up for payment tracking without re-entering it by hand. This is a
-- one-way, creation-time-only link — editing a PDF invoice afterward does
-- not update its tracker row.
...
BEGIN
  ...
  LOOP
    ...
    IF today_day >= effective_day THEN
      ...
      -- Link into the Invoices tracker as Pending, same as a manually
      -- created Invoice Builder invoice.
